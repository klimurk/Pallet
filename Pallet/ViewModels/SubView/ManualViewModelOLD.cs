...
namespace Pallet.ViewModels.SubView
{
    public class ManualViewModelOLD : ViewModel
    {
        ...
        public ManualViewModelOLD(IManagerNailTypes ManagerNailTypes, IManagerUser ManagerUser, IOPC OPC)
        {
        ...
        private bool CanSetRobotMoveLeftCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_Links").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetRobotMoveLeftCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_Links").Value = true;
        ...
        private bool CanResetRobotMoveLeftCommandExecute(object arg) => (bool)_Signals.First(s => s.Name == "M_Links").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnResetRobotMoveLeftCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_Links").Value = false;
        ...
        private bool CanSetRobotMoveRightCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_rechts").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetRobotMoveRightCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_rechts").Value = true;
        ...
        private bool CanResetRobotMoveRightCommandExecute(object arg) => (bool)_Signals.First(s => s.Name == "M_rechts").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnResetRobotMoveRightCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_rechts").Value = false;
        ...
        private bool CanSetRobotMoveUpCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_up").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetRobotMoveUpCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_up").Value = true;
        ...
        private bool CanResetRobotMoveUpCommandExecute(object arg) => (bool)_Signals.First(s => s.Name == "M_up").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnResetRobotMoveUpCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_up").Value = false;
        ...
        private bool CanSetRobotMoveDownCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_down").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetRobotMoveDownCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_down").Value = true;
        ...
        private bool CanResetRobotMoveDownCommandExecute(object arg) => (bool)_Signals.First(s => s.Name == "M_down").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnResetRobotMoveDownCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_down").Value = false;
        ...
        private bool CanSetLampTestCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_LampTeste").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetLampTestCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_LampTeste").Value = true;
        ...
        private bool CanResetLampTestCommandExecute(object arg) => (bool)_Signals.First(s => s.Name == "M_LampTeste").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnResetLampTestCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_LampTeste").Value = false;
        ...
        private bool CanSetRobotNailShootCommandExecute(object arg) => !(bool)_Signals.First(s => s.Name == "M_shooting").Value && _UserManager.LoginedUser?.RoleNum >= (int)IManagerUser.UserRoleNum.Worker;

        private void OnSetRobotNailShootCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_nail").Value = true;
        ...
        private void OnSetMoveToConveyorCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_MoveToConveyor").Value = true;
        ...
        private void OnSetRobotToHomePositionCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_Home").Value = true;
        ...
        private void OnRotateTableCommandExecuted(object obj) => _Signals.First(s => s.Name == "M_TableRotate").Value = true;
