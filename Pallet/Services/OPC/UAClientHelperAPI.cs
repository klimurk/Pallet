...
public class UAClientHelperAPI
{
    ...
    public bool errorConnect = false;
    ...
    /// </summary>
    private readonly ApplicationConfiguration mApplicationConfig = null;
    ...
    public ApplicationDescriptionCollection FindServers(string discoveryUrl)
    {
        ...
        catch (Exception e)
        {
            ...
            //throw e;
            if (countRepeat >= 3) { errorConnect = true; return null; }
    ...
    /// <param name="password">The password</param>
    /// <exception cref="Exception">Throws and forwards any exception with short error description.</exception>
    public async Task Connect(EndpointDescription endpointDescription, bool userAuth, uint SessionTimeout = 60000, string userName = default, string password = default)
    {
        try
        {
            //Secify application configuration
            ...
            ConfiguredEndpoint mEndpoint = new(null, endpointDescription, EndpointConfiguration);
    ...
    public uint GetNamespaceIndex(String uri)
    {
        ...
        {
            Exception e = new("Namespace doesn't exist");
            throw e;
        }
    }
    ...
    public String GetNamespaceUri(uint index)
    {
        ...
        {
            Exception e = new("Index is out of range");
            throw e;
        }
    }
    ...
    public Node ReadNode(String nodeIdString)
    {
        ...
        {
            //Read the dataValue
            return mSession.ReadNode(new NodeId(nodeIdString));
        }
        catch (Exception e)
        {
            errorConnect = true; return null;
    ...
    public List<string> ReadValues(List<String> nodeIdStrings)
    {
        ...
        try
        {
            ...
            foreach (ServiceResult svResult in serviceResults)
            {
                ...
                {
                    Exception e = new(svResult.ToString());
                    throw e;
                }
            }
            ...
            foreach (object result in values)
            {
                if (result != null)
                {
                    ...
                    {
                        string str = "";
                        foreach (long intVar in (long[])result)
                        {
                            str = str + ";" + intVar.ToString();
                        }
                        str = str.Remove(0, 1);
    ...
    public void WriteValues(List<String> values, List<String> nodeIdStrings)
    {
        ...
        try
        {
            ...
            foreach (StatusCode code in result)
            {
                if (code != 0)
                {
                    Exception ex = new(code.ToString());
                    throw ex;
                }
            }
        }
    ...
    public List<string[]> ReadStructUdt(String nodeIdString)
    {
        ...
        //Read the struct
        List<NodeId> nodeIds = new();
        NodeId myNode = new(nodeIdString);
        nodeIds.Add(myNode);
        List<ServiceResult> serviceResults = new();
        List<Object> values = new();
        List<Type> types = new();
        ...
        foreach (object val in values)
        {
            ...
            if (val is not ExtensionObject encodeable)
            {
    ...
    public void WriteStructUdt(String nodeIdString, List<string[]> dataToWrite)
    {
        ...
        if (!dataToWrite[0][2].Contains("OF STRUCT/UDT"))
        {
            ...
            long length = GetLengthOfDataToWrite(dataToWrite);
        ...
        else
        {
            ...
            for (int i = 0; i < arrDim; i++)
            {
                ...
                //Determine lentgh of byte array needed to contain all data
                long length = GetLengthOfDataToWrite(splitData);
    ...
    #endregion Read/Write Struct/UDT
    ...
    public IList<object> CallMethod(String methodIdString, String objectIdString, List<string[]> inputData)
    {
        ...
        //Call the method
        return mSession.Call(objectNodeId, methodNodeId, inputArguments);
    }
                        ...
                        if (refDesc.DisplayName.Text == "InputArguments" || (refDesc.DisplayName.Text == "OutputArguments" && refDesc.NodeClass == NodeClass.Variable))
                        {
                            ...
                            foreach (object result in values)
                            {
                                if (result != null)
                                {
                                    ...
                                    if (result is not ExtensionObject encodeable)
                                    {
    ...
    private void Notificatio_CertificateValidation(CertificateValidator certificate, CertificateValidationEventArgs e) => CertificateValidationNotification(certificate, e);

    /// <summary>Eventhandler for KeepAlive forwards this event</summary>
    private void Notification_KeepAlive(Session session, KeepAliveEventArgs e) => KeepAliveNotification(session, e);
    ...
    private void Notification_MonitoredItem(MonitoredItem monitoredItem, MonitoredItemNotificationEventArgs e) => ItemChangedNotification(monitoredItem, e);
    ...
    private static long GetLengthOfDataToWrite(List<string[]> dataToWrite)
    {
        ...
        foreach (string[] val in dataToWrite)
        {
            ...
            {
                Exception e = new("Unknown data type. Can't determine length of data");
                throw e;
            }
        }
        return length;
    ...
    /// <exception cref="Exception">Throws and forwards any exception with short error description.</exception>
    private static byte[] ParseDataToByteArray(List<string[]> dataToWrite, long dataLength)
    {
    ...
    private static List<string[]> ParseDataToTagsFromDictionary(List<Object> varList, byte[] byteResult)
    {
        ...
        foreach (object val in varList)
        {
            ...
            {
                Exception e = new("Data type is too complex to be parsed." + System.Environment.NewLine + "Please avoid array of UDT/Struct inside UDT/Struct");
                throw e;
            }
